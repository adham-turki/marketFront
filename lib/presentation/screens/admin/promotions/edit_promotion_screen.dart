...
class EditPromotionScreen extends StatefulWidget {
  ...
  final Function(Map<String, dynamic>) onPromotionUpdated;
...
class _EditPromotionScreenState extends State<EditPromotionScreen> {
  ...
  Future<void> _updatePromotion() async {
    ...
    try {
      ...
      print('Updating promotion data: $promotionData');

      final response = await _apiService
          .put('/promotions/${widget.promotion['id']}', data: promotionData);

      if (response.statusCode == 200 || response.statusCode == 201) {
        print('DEBUG: Promotion updated successfully, calling callback');
        // First call the callback to update the promotions page
        widget.onPromotionUpdated(response.data['promotion'] ?? response.data);
        ...
        _showSnackBar(
            response.data['message'] ?? ArabicText.errorUpdatingPromotion,
            isError: true);
      }
    } catch (e) {
      _showSnackBar('${ArabicText.errorUpdatingPromotion}: $e', isError: true);
    } finally {
      setState(() {
        _isLoading = false;
      });
